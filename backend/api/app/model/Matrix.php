...
class Matrix {
	...
	private function getMatrixHeaders($instrumentId) {
		...
		$lastGroupId = NULL;
		$groups = [];
		$groupRecords = $this->database->table('question_group')->where('instrument_id=?', $instrumentId)->order('sort_order');
		$groupNumbers = [];
		foreach ($groupRecords as $groupRecord) {
			$groups[] = trim($groupRecord["number"] . ' ' . $groupRecord["tag"]);
			$groupNumbers[$groupRecord["id"]] = trim($groupRecord["number"]);
		}
		$questionRecords = $this->database->table('question')
	...
	public function myMatrix($organizationId, $instrumentId) {
		$childCount = $this->database->table('organization')->where('parent_id=?', $organizationId)->count();
		if ($childCount > 0) {
