...
class Matrix {
	...
	function __construct($api, $pdo, $db) {
		$this->api = $api;
		$this->pdo = $pdo;
		$this->db = $db;
	}
	...
	private function getMembersForAssessments($organizationId, $instrumentId) {
		$dbRecords = $this->db->assessment()->select('member_id,max(last_modified) as maxMod')
	...
	private function getMatrixHeaders($instrumentId) {
		...
		$lastGroupId = NULL;
		$groups = [];
		$groupRecords = $this->db->question_group()->where('instrument_id=?', $instrumentId)->order('sort_order');
		foreach ($groupRecords as $groupRecord) {
			$groups[] = trim($groupRecord["number"] . ' ' . $groupRecord["tag"]);
		}
		$questionRecords = $this->db->question()
	...
	public function myMatrix($organizationId, $instrumentId) {
		$childCount = $this->db->organization()->where('parent_id=?',[$organizationId])->count();
		if ($childCount > 0) {
