...
class Matrix {
    ...
    private function getMatrixHeaders($instrumentId) {
        ...
        foreach ($groupRecords as $groupRecord) {
            $groups[] = trim($groupRecord->number . ' ' . $groupRecord->name);
            ...
            foreach ($questionRecords as $questionRecord) {
                ...
                $fullName = $groupNumbers[$groupId] . '.' . trim($questionRecord->number . ' ' . $questionRecord->name);
                if ($lastGroupId != NULL && $groupId != $lastGroupId && $groupIdx < count($groups)) {
                    $headers[] = ['S', $groups[$groupIdx], 'H', $groupIdx];
                    $groupIdx++;
                }
                $lastGroupId = $groupId;
                $headers[] = ['R', $fullName, 'H', $groupIdx];
            }
        }
        if (count($groups) > 0) {
    ...
    public function buildOrgMatrix($organizationId, $instrumentId) {
        ...
        $sql = "SELECT 	
				...
				WHERE 	
				    retrieveOrgSubParentId($organizationId,m.organization_id) > 0 AND
