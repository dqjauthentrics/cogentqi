...
(function (window) {
	...
	// dqj - moved color array to shared array
	Colors.colorSet = {
		"aliceblue": "#f0f8ff",
		"antiquewhite": "#faebd7",
		"aqua": "#00ffff",
	...
	// dqj - added method
	// Returns set of hex value colors, typically used for looping.
	//
	Colors.getHexSet = function () {
		var keys = [];
		for (var key in Colors.colorSet) {
			keys.push(Colors.colorSet[key]);
		}
		return keys;
	};
	...
	// dqj - from https://24ways.org/2010/calculating-color-contrast/
	Colors.contrast50 = function (hexColor) {
		var hexStr = hexColor.substr(1);
		return (parseInt(hexStr, 16) > 0xffffff / 2) ? '#000000' : '#FFFFFF';
	};

	// dqj - from https://24ways.org/2010/calculating-color-contrast/
	Colors.contrastYIQ = function (hexColor) {
		var hexStr = hexColor.substr(1);
		var rgb = this.hex2rgb(hexStr);
		var yiq = ((rgb['R'] * 299) + (rgb['G'] * 587) + (rgb['B'] * 114)) / 1000;
		return (yiq >= 128) ? '#000000' : '#FFFFFF';
	};

	// dqj - from http://stackoverflow.com/questions/43044/algorithm-to-randomly-generate-an-aesthetically-pleasing-color-palette
	Colors.pastel = function (hexColor, mixLevel) {
		if (typeof mixlevel == 'undefined' || mixLevel == null) {
			mixLevel = 220;
		}
		var hexStr = hexColor.substr(1);
		var rgb = this.hex2rgb(hexStr);
		rgb['R'] = Math.floor((rgb['R'] + mixLevel) / 2);
		rgb['G'] = Math.floor((rgb['G'] + mixLevel) / 2);
		rgb['B'] = Math.floor((rgb['B'] + mixLevel) / 2);
